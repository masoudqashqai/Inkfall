// ---- AFTERMATH — the dead and what they leave: body, chalk outline, blood splat, drain ----
import { Noir } from '../core/engine.js';
import { PALETTE } from '../core/palette.js';
import { TWO_PI, rand32, lerp, smooth01 } from '../core/math.js';
...
// blood that spreads from the body and crawls to the drain — only from p.drainAt onward (a later beat),
// drawn as an organic widening rivulet of overlapping blobs that thins to watery pink near the drain
Noir.registerActor('bloodDrain', (e, p) => {
  if (!e.flags.blood) return;
  const drainAt = p.drainAt != null ? p.drainAt : 0;
  if (e.lineIdx < drainAt) return;
  const amt = e.lineIdx > drainAt ? 1 : smooth01(e.beat() / 5);
  const c = e.ctx, par = e.parallax * (p.par == null ? 0.5 : p.par);
  const x = p.x * e.W + par, y = e.gy + 12;
  const dx = (p.drainX != null ? p.drainX : 0.5) * e.W + par, dy = (p.drainY != null ? p.drainY * e.H : e.gy + (e.H - e.gy) * 0.42);
  const cx2 = (x + dx) / 2, cy2 = Math.max(y, dy) + 16, reach = Math.min(1, amt), N = 46;
  c.save();
  c.fillStyle = 'rgba(116,0,12,0.82)'; c.beginPath(); c.ellipse(x, y, 14 + 12 * amt, 5 + 4 * amt, 0, 0, TWO_PI); c.fill();   // spreading source pool
  for (let i = 0; i <= N; i++) {
  ...
  if (reach > 0.8) { c.fillStyle = 'rgba(170,30,50,0.42)'; c.beginPath(); c.ellipse(dx, dy, 11, 4.4, 0, 0, TWO_PI); c.fill(); }   // watery pool at the drain
  c.restore();
});
