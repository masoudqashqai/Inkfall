// ---- PEOPLE — the noir cast: detective, heavy, boss, shooter, femme fatale, singer ----
import { Noir } from '../core/engine.js';
import { PALETTE } from '../core/palette.js';
import { ANIM } from '../core/anim.js';
import { TWO_PI, lerp, smooth01 } from '../core/math.js';
import { rimSign, bodyGrad, shadowPool, ember, cigSmoke, drawFedora, drawPistol, muzzleFlash } from './helpers.js';

Noir.registerActor('trenchMan', (e, p) => {                 // the detective: trench coat + fedora
  const c = e.ctx, s = e.scaleOf(p), X = e.X(p), gy = e.gy + (p.dy || 0) * e.unit, t = e.t, rim = rimSign(e, p), tint = e.litTint(X), sway = Math.sin(t * ANIM.swaySpeed) * 1.2;
  ...
  // right arm raises a cigarette on p.raiseAt and lights it on p.lightAt; before that it hangs at his side
  ...
  const ex = lerp(16 * s, 26 * s, aim), ey = lerp(-60 * s, -73 * s, aim), hx = lerp(17 * s, 13 * s, aim), hy = lerp(-44 * s, -101 * s, aim);
  c.lineCap = 'round'; c.lineJoin = 'round';
  c.strokeStyle = '#2a2f37'; c.lineWidth = 8.5 * s;
  c.beginPath(); c.moveTo(11 * s, -87 * s); c.lineTo(ex, ey); c.lineTo(hx, hy); c.stroke();   // right arm (shoulder→elbow→hand)
  c.beginPath(); c.moveTo(-13 * s, -87 * s); c.lineTo(-21 * s, -50 * s); c.stroke();           // left arm
  c.strokeStyle = 'rgba(150,160,178,0.3)'; c.lineWidth = 1.4 * s; c.beginPath(); c.moveTo(12 * s, -90 * s); c.lineTo(ex, ey); c.stroke();
  c.fillStyle = '#bcbab0'; c.beginPath(); c.arc(hx, hy, 2.9 * s, 0, TWO_PI); c.arc(-21 * s, -48 * s, 3.2 * s, 0, TWO_PI); c.fill();   // hands
  ...
  if (aim > 0.5) {                                          // a cigarette in the raised hand
  ...
  c.restore();
  // the lit cigarette / striking Zippo is a small warm light source (air + wet floor + rain)
  if (aim > 0.5 && litCig) {
    const cwx = X + sway + hx + 9 * s, cwy = gy + hy, I = 0.3 + flash * 0.6;
    e.lightHalo(cwx, cwy, '255,150,60', 84 * s, I); e.pushLight(cwx, cwy, '255,150,60', 92 * s, I * 0.7); e.floorRefl(cwx, '255,150,60', I * 0.6, 14, 0);
  }
});
...
Noir.registerActor('gunman', (e, p) => {                    // shooter, arm extended, muzzle flash on 'muzzle'
  const c = e.ctx, s = e.scaleOf(p), X = e.X(p), gy = e.gy + (p.dy || 0) * e.unit, t = e.t, rim = rimSign(e, p), tint = e.litTint(X), flash = e.flags.muzzle || 0;
  ...
  // arm: p.raiseAt lowers it until that line, then it lifts and levels the pistol for the shots
  const aim = (p.raiseAt != null) ? (e.lineIdx < p.raiseAt ? 0 : (e.lineIdx === p.raiseAt ? smooth01(e.beat() / 2.4) : 1)) : 1;
...
Noir.registerActor('womanInRed', (e, p) => {                // femme fatale — the colour that bleeds
  const c = e.ctx, s = e.scaleOf(p), X = e.walkX(p), gy = e.gy + (p.dy || 0) * e.unit, t = e.t, walk = Math.sin(t * ANIM.walkSpeed);
  // her heels run as a continuous loop ONLY while she is actually moving between two different marks
  if (p.walk) { const i = Math.min(e.lineIdx, p.walk.length - 1), prev = i > 0 ? p.walk[i - 1] : p.walk[0]; e.walkSound(prev !== p.walk[i] && e.beat() < (p.walkDur || 3.4) - 0.2); }
  // her wet reflection fades out while she passes the man, so it never pools on the ground under him
  let reflA = 1;
  if (p.passX != null) { const manX = p.passX * e.W + e.parallax * (p.par == null ? 0.5 : p.par); reflA = smooth01((Math.abs(X - manX) - 20 * s) / (40 * s)); }
