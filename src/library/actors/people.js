...
function trenchPose(e, p) {
  ...
  const hx = lerp(23 * s, 13 * s, aim), hy = lerp(-38 * s, -101 * s, aim);   // resting hand hangs wide + low at his side; swings in/up to the mouth as he raises the cigarette
...
defineActor('trenchMan', function (e) {                       // detective: trench coat + fedora
  ...
  const ex = lerp(20 * s, 26 * s, aim), ey = lerp(-62 * s, -73 * s, aim), hx = P.hx, hy = P.hy;
  c.lineCap = 'round'; c.lineJoin = 'round';
  c.strokeStyle = '#2a2f37'; c.lineWidth = 8.5 * s;
  c.beginPath(); c.moveTo(16 * s, -86 * s); c.lineTo(ex, ey); c.lineTo(hx, hy); c.stroke();   // cigarette arm: off the shoulder, out to the side, up to the mouth as it raises
  c.beginPath(); c.moveTo(-16 * s, -86 * s); c.lineTo(-23 * s, -38 * s); c.stroke();           // other arm hangs straight + wide at his side
  c.strokeStyle = 'rgba(150,160,178,0.3)'; c.lineWidth = 1.4 * s; c.beginPath(); c.moveTo(16 * s, -88 * s); c.lineTo(ex, ey); c.stroke();
  c.fillStyle = '#bcbab0'; c.beginPath(); c.arc(hx, hy, 2.9 * s, 0, TWO_PI); c.arc(-23 * s, -38 * s, 3.2 * s, 0, TWO_PI); c.fill();
