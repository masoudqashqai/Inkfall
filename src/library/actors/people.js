...
import { albedo, ember, cigSmoke, drawFedora, drawPistol, muzzleFlash } from '../shared.js';

// cloth albedos — now HONEST material colours (a real tan trench, a navy suit). The compositor
// lights them as a whole and the noir grade in post pulls the frame toward black and white, so they
// do not need to be pre-greyed here.
const TRENCH = [168, 142, 100], HEAVY = [78, 86, 102], PINSTRIPE = [46, 50, 70], SUIT = [58, 68, 92];
// bright surface albedos: shirt linen, warm skin, the motif red. The grade keeps the saturated red
// bleeding while the skin and shirt read mostly neutral once graded.
const SHIRT = [228, 224, 210], BONE = [232, 198, 168], HAND = [220, 188, 160], RED = [214, 32, 40];
...
defineActor('trenchMan', function (e) {                       // detective: trench coat + fedora
  ...
  c.fillStyle = albedo(HAND); c.beginPath(); c.arc(hx, hy, 2.9 * s, 0, TWO_PI); c.arc(-16 * s, -50 * s, 2.9 * s, 0, TWO_PI); c.fill();
  c.fillStyle = albedo(TRENCH);
  ...
  drawFedora(c, 0, -120 * s, 13 * s);
  if (aim > 0.5) { c.save(); c.translate(hx, hy); c.fillStyle = albedo([216, 210, 196]); c.fillRect(2 * s, -1 * s, 7 * s, 2 * s); c.restore(); }   // the cigarette stick (albedo)
  c.restore();
}, {
  shadowW: 22, shadowH: 130,
  // EMISSIVE: the cigarette smoke, ember and the lighter flame, painted over the lit figure so they
  // stay bright (the scene glow they throw comes from emitLight, the light system).
  glow(e) {
    const c = e.ctx, P = trenchPose(e, this), s = P.s, t = P.t;
    if (P.aim <= 0.5) return;
    c.save(); c.translate(P.X + P.sway, P.gy);
    const hx = P.hx, hy = P.hy;
    if (P.litCig) { cigSmoke(c, hx + 9 * s, hy, s, t); ember(c, hx + 9 * s, hy, s, t); }
    if (P.flash > 0) {
      const lx = hx + 9 * s, ly = hy, fh = (7 + Math.sin(t * 40) * 1.6) * s * (0.5 + 0.5 * P.flash);
      c.globalCompositeOperation = 'lighter';
      c.fillStyle = `rgba(255,140,40,${0.85 * P.flash})`; c.beginPath(); c.moveTo(lx - 2.4 * s, ly); c.quadraticCurveTo(lx - 2.2 * s, ly - fh * 0.6, lx, ly - fh); c.quadraticCurveTo(lx + 2.2 * s, ly - fh * 0.6, lx + 2.4 * s, ly); c.quadraticCurveTo(lx, ly + 1.6 * s, lx - 2.4 * s, ly); c.fill();
      c.fillStyle = `rgba(255,242,190,${0.95 * P.flash})`; c.beginPath(); c.moveTo(lx - 1.1 * s, ly); c.quadraticCurveTo(lx - 1 * s, ly - fh * 0.45, lx, ly - fh * 0.7); c.quadraticCurveTo(lx + 1 * s, ly - fh * 0.45, lx + 1.1 * s, ly); c.quadraticCurveTo(lx, ly + s, lx - 1.1 * s, ly); c.fill();
    }
    c.restore();
  },
...
defineActor('thug', function (e) {                            // the heavy: broad, flat cap, big fists
  ...
  c.fillStyle = albedo(SHIRT); c.beginPath(); c.moveTo(-6 * s, -82 * s); c.lineTo(6 * s, -82 * s); c.lineTo(3 * s, -54 * s); c.lineTo(-3 * s, -54 * s); c.closePath(); c.fill();
  c.fillStyle = albedo(RED); c.beginPath(); c.moveTo(-2.5 * s, -80 * s); c.lineTo(2.5 * s, -80 * s); c.lineTo(1.5 * s, -56 * s); c.lineTo(-1.5 * s, -56 * s); c.closePath(); c.fill();
  c.fillStyle = albedo(HEAVY); c.beginPath(); c.moveTo(-12 * s, -84 * s); c.lineTo(-6 * s, -82 * s); c.lineTo(-7 * s, -52 * s); c.lineTo(-16 * s, -56 * s); c.closePath(); c.moveTo(12 * s, -84 * s); c.lineTo(6 * s, -82 * s); c.lineTo(7 * s, -52 * s); c.lineTo(16 * s, -56 * s); c.closePath(); c.fill();
...
defineActor('boss', function (e) {                            // mob boss: pinstripe suit, fedora, cigar
  ...
  c.fillStyle = albedo(SHIRT); c.beginPath(); c.moveTo(-5 * s, -88 * s); c.lineTo(5 * s, -88 * s); c.lineTo(3 * s, -60 * s); c.lineTo(-3 * s, -60 * s); c.closePath(); c.fill();
  c.fillStyle = albedo(RED); c.beginPath(); c.moveTo(-2.5 * s, -86 * s); c.lineTo(2.5 * s, -86 * s); c.lineTo(1.4 * s, -58 * s); c.lineTo(-1.4 * s, -58 * s); c.closePath(); c.fill();
  ...
  drawFedora(c, 0, -116 * s, 12 * s);
  c.fillStyle = '#3a2a1c'; c.fillRect(6 * s, -104 * s, 13 * s, 3 * s);   // the cigar (albedo)
  c.restore();
}, {
  shadowW: 24, shadowH: 126,
  glow(e) {                                                   // EMISSIVE: cigar smoke + the lit tip
    const c = e.ctx, s = e.scaleOf(this), X = e.X(this), gy = e.gy + (this.dy || 0) * e.unit, t = e.t;
    c.save(); c.translate(X, gy);
    cigSmoke(c, 19 * s, -104 * s, s, t);
    c.fillStyle = 'rgba(255,80,30,0.95)'; c.shadowColor = PALETTE.ember; c.shadowBlur = 12; c.beginPath(); c.arc(19 * s, -102.5 * s, 2.4 * s, 0, TWO_PI); c.fill();
    c.restore();
  },
...
defineActor('gunman', function (e) {                          // shooter, arm extended, muzzle flash on 'muzzle'
  ...
  glow(e) {                                                   // EMISSIVE: the muzzle flash at the barrel
    const flash = e.flags.muzzle || 0; if (flash <= 0) return;
    const c = e.ctx, s = e.scaleOf(this), X = e.X(this), gy = e.gy + (this.dy || 0) * e.unit;
    const aim = (this.raiseAt != null) ? (e.lineIdx < this.raiseAt ? 0 : (e.lineIdx === this.raiseAt ? smooth01(e.beat() / 2.4) : 1)) : 1;
    if (aim <= 0.82) return;
    c.save(); c.translate(X, gy); if (this.flip) c.scale(-1, 1);
    const sx = 6 * s, sy = -86 * s, hx = lerp(13 * s, 34 * s, aim), hy = lerp(-52 * s, -82 * s, aim), ang = Math.atan2(hy - sy, hx - sx);
    muzzleFlash(c, hx + Math.cos(ang) * 18 * s, hy + Math.sin(ang) * 18 * s, s, flash);
    c.restore();
  },
...
defineActor('womanInRed', function (e) {                      // femme fatale — the colour that bleeds
  ...
  const sw = walk * 2 * s;   // sw: the hem drifts as she moves
  c.save(); c.translate(X, gy);
  // a leg through the thigh slit (bone) + a red heel
  c.fillStyle = albedo(BONE); c.save(); c.translate(walk * 2 * s, 0); c.beginPath(); c.moveTo(-2 * s, -36 * s); c.lineTo(3 * s, -36 * s); c.lineTo(4 * s, -2 * s); c.lineTo(0, -2 * s); c.closePath(); c.fill(); c.restore();
  c.fillStyle = albedo(RED); c.beginPath(); c.moveTo(sw, -2 * s); c.lineTo(8 * s + sw, 0); c.lineTo(sw, 1 * s); c.closePath(); c.fill();
  // slim bone arms with a soft bend, hands resting at the waist
  c.strokeStyle = albedo(BONE); c.lineWidth = 3.2 * s; c.lineCap = 'round'; c.lineJoin = 'round';
  c.beginPath(); c.moveTo(-7 * s, -70 * s); c.lineTo(-12 * s, -56 * s); c.lineTo(-9 * s, -44 * s); c.stroke();
  c.beginPath(); c.moveTo(7 * s, -70 * s); c.lineTo(12 * s, -56 * s); c.lineTo(9 * s, -44 * s); c.stroke();
  // a slim neck (bone) so the head meets the gown. Same albedo as the face, the compositor lights the
  // whole figure as one, so neck, arms and face all shade together (no per-part mismatch).
  c.fillStyle = albedo(BONE); c.beginPath(); c.moveTo(-2.5 * s, -80 * s); c.lineTo(2.5 * s, -80 * s); c.lineTo(2 * s, -68 * s); c.lineTo(-2 * s, -68 * s); c.closePath(); c.fill();
  // the gown: the motif red albedo, lit as a whole by the compositor (sweetheart neckline, cinched
  // waist, curvy hips, a long mermaid skirt). Stays vivid through the grade, the colour that bleeds.
  c.fillStyle = albedo([200, 26, 34]);
  c.beginPath();
  ...
  c.fillStyle = albedo(BONE); c.beginPath(); c.arc(0, -84 * s, 6.5 * s, 0, TWO_PI); c.fill();
  ...
  c.fillStyle = albedo(RED); c.beginPath(); c.ellipse(-1 * s, -81 * s, 2.2 * s, 1.1 * s, 0, 0, TWO_PI); c.fill();
  c.restore();
}, {
  shadowW: 15, shadowH: 91, spec: 0.18,                       // satin gown gives a soft sheen
...
defineActor('dealer', function (e) {                          // croupier behind the table (the felt covers his lower half)
  ...
  c.fillStyle = albedo([226, 222, 210]); c.beginPath(); c.moveTo(-4 * s, -66 * s); c.lineTo(4 * s, -66 * s); c.lineTo(3 * s, -30 * s); c.lineTo(-3 * s, -30 * s); c.closePath(); c.fill();
  ...
  c.fillStyle = albedo(RED); c.beginPath(); c.moveTo(-4.5 * s, -67 * s); c.lineTo(0, -64 * s); c.lineTo(-4.5 * s, -61 * s); c.closePath(); c.moveTo(4.5 * s, -67 * s); c.lineTo(0, -64 * s); c.lineTo(4.5 * s, -61 * s); c.closePath(); c.fill();
  ...
  c.fillStyle = albedo([210, 186, 162]); c.beginPath(); c.arc(-23 * s, -28 * s, 3.5 * s, 0, TWO_PI); c.arc(23 * s, -28 * s, 3.5 * s, 0, TWO_PI); c.fill();
...
defineActor('singer', function (e) {                          // lounge singer at the mic, in a spotlight
  const c = e.ctx, s = e.scaleOf(this), X = e.X(this), gy = e.gy + (this.dy || 0) * e.unit;
  const gown = albedo(this.red ? [200, 26, 34] : [212, 206, 196]);
  c.save(); c.translate(X, gy);
  c.fillStyle = gown;
  ...
  c.beginPath(); c.moveTo(-6 * s, -66 * s); c.quadraticCurveTo(-7 * s, -78 * s, -4 * s, -84 * s); c.lineTo(4 * s, -84 * s); c.quadraticCurveTo(7 * s, -78 * s, 6 * s, -66 * s); c.closePath(); c.fill();
  c.strokeStyle = gown; c.lineWidth = 5 * s; c.lineCap = 'round'; c.beginPath(); c.moveTo(4 * s, -80 * s); c.lineTo(16 * s, -92 * s); c.stroke();
  c.fillStyle = albedo(BONE); c.beginPath(); c.arc(16 * s, -92 * s, 3.5 * s, 0, TWO_PI); c.fill();
  c.fillStyle = albedo(BONE); c.beginPath(); c.arc(0, -92 * s, 6.5 * s, 0, TWO_PI); c.fill();
  c.fillStyle = '#070708'; c.beginPath(); c.arc(0, -94 * s, 8 * s, Math.PI * 0.85, Math.PI * 2.25); c.fill();
  c.fillStyle = albedo(RED); c.beginPath(); c.ellipse(0, -89 * s, 2.2 * s, 1.1 * s, 0, 0, TWO_PI); c.fill();
  ...
  shadowW: 16, shadowH: 90, spec: 0.18,                       // satin gown sheen under the spotlight
