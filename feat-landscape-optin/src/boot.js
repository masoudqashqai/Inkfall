...
const BUILD = 'v2 build 60 · landscape opt-in (no force)'
...
// Landscape is OFFERED, never forced. On a phone we ask once ("go landscape & fullscreen, or stay
// in portrait?"), remember the answer, and either way the engine just letterboxes — no blocking
// rotate gate. Only the HUD fullscreen toggle is wired to the viewport; the gate overlay is not.
const viewport = new Viewport(engine);
viewport.attach({ fsBtn: el('fsbtn') });
viewport.onStart = () => manager.requestStart();
viewport.onPause = () => Audio2.suspend();     // full story pause only for the menu/modal, not orientation
...
// start the story. `goLandscape` requests fullscreen + landscape (a no-op where unsupported, e.g.
// iPhone Safari — the user can still rotate by hand and it letterboxes). No orientation gate: the
// story plays immediately, portrait or landscape.
const ORIENT_KEY = 'inkfall_orient';
function startPlaying(goLandscape) {
  el('rotate').classList.remove('show');
  viewport.experienceStarted = true; viewport.gatePassed = true;   // a choice was made; never raise a blocking gate
  if (goLandscape) viewport.enter();                               // must fire inside the click gesture
  setStage('playing');                                            // CSS reveals the HUD + narration, fades the intro
  viewport.evaluate();                                            // starts the story now
}

// ENTER: on a phone that hasn't chosen yet, ask once; otherwise (desktop, or remembered) just play.
function beginPlay() {
  let pref = null; try { pref = localStorage.getItem(ORIENT_KEY); } catch (e) {}
  if (viewport.rotatable && !pref) { el('rotate').classList.add('show'); return; }
  startPlaying(pref === 'landscape');
}
function choose(goLandscape) { try { localStorage.setItem(ORIENT_KEY, goLandscape ? 'landscape' : 'portrait'); } catch (e) {} startPlaying(goLandscape); }
el('orient-yes').addEventListener('click', () => choose(true));
el('orient-no').addEventListener('click', () => choose(false));
