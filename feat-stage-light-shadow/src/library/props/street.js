...
defineProp('barrelFire', function (e) {                       // a drum fire in the alley
  ...
  // only the drum (albedo) is drawn here. The flames + sparks are emissive (glow), the warm glow in
  // the air comes from the lighting system (emitLight + air glow + floor wash).
  ...
  glow(e) {                                                   // EMISSIVE: the flames and rising sparks
...
defineProp('trafficLight', function (e) {                     // turns green on greenAt; a coloured light source
  const c = e.ctx, s = e.scaleOf(this), X = e.X(this), gy = e.gy + (this.dy || 0) * e.unit;
  c.save(); c.translate(X, gy);
  c.fillStyle = '#191c22'; c.fillRect(-2.5 * s, -150 * s, 5 * s, 150 * s);                 // pole
  ...
  // the three lenses, dark (the lit one is drawn bright in glow)
  c.fillStyle = 'rgba(70,70,74,0.5)'; for (const yy of [-190, -175, -160]) { c.beginPath(); c.arc(0, yy * s, 5 * s, 0, TWO_PI); c.fill(); }
  c.restore();
}, {
  glow(e) {                                                   // EMISSIVE: the active lens
    const c = e.ctx, s = e.scaleOf(this), X = e.X(this), gy = e.gy + (this.dy || 0) * e.unit;
    const green = (this.greenAt != null && e.lineIdx >= this.greenAt);
    c.save(); c.translate(X, gy); c.fillStyle = green ? '#36d36e' : '#ff2a2a'; c.shadowColor = c.fillStyle; c.shadowBlur = 16 * s;
    c.beginPath(); c.arc(0, (green ? -160 : -190) * s, 5 * s, 0, TWO_PI); c.fill(); c.restore();
  },
  emitLight(e) {
...
defineProp('fireHydrant', function (e) {
  const c = e.ctx, s = e.scaleOf(this) * 1.3, X = e.X(this), gy = e.gy + (this.dy || 0) * e.unit;   // 1.3: was knee height, now reads like a real hydrant
  c.save(); c.translate(X, gy);
  ...
  castsShadow: true, shadowW: 12, shadowH: 34,
  shadowSil(e, c) {
    const s = e.scaleOf(this) * 1.3;
...
defineProp('streetSign', function (e) {
  const c = e.ctx, s = e.scaleOf(this) * 1.25, X = e.X(this), gy = e.gy + (this.dy || 0) * e.unit;   // 1.25: sits above head height like a real street sign
  c.save(); c.translate(X, gy);
  ...
  castsShadow: true, shadowW: 38, shadowH: 154,
  shadowSil(e, c) { const s = e.scaleOf(this) * 1.25; c.fillRect(-1.5 * s, -120 * s, 3 * s, 120 * s); c.fillRect(-30 * s, -123 * s, 60 * s, 12 * s); },
});
...
defineProp('dumpster', function (e) {
  ...
  c.save();
  c.fillStyle = albedo([46, 64, 54]); c.fillRect(X - bw / 2, g - bh, bw, bh);
  c.fillStyle = albedo([62, 82, 72]); c.fillRect(X - bw / 2 - 3 * s, g - bh - 3 * s, bw + 6 * s, 3.5 * s);
