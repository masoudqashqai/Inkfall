...
defineActor('womanInBlack', function (e) {                    // the Sable Dame — black gown, pale skin, red lips
  const c = e.ctx, s = e.scaleOf(this), X = e.walkX(this), gy = e.gy + (this.dy || 0) * e.unit, t = e.t, walk = Math.sin(t * ANIM.walkSpeed) * (this.walk ? 1 : 0.5);
  if (this.walk) { const i = Math.min(e.lineIdx, this.walk.length - 1), prev = i > 0 ? this.walk[i - 1] : this.walk[0]; e.walkSound(prev !== this.walk[i] && e.beat() < (this.walkDur || 3.4) - 0.2); }
  e.groundShadow(X, 13 * s, 84 * s);
  c.save(); c.translate(X, gy);
  c.globalAlpha = 0.2; const grd = c.createLinearGradient(0, 0, 0, 40 * s); grd.addColorStop(0, 'rgba(150,160,180,0.4)'); grd.addColorStop(1, 'rgba(150,160,180,0)'); c.fillStyle = grd; c.fillRect(-12 * s, 0, 24 * s, 42 * s); c.globalAlpha = 1;
  c.fillStyle = PALETTE.bone; c.save(); c.translate(walk * 2 * s, 0); c.beginPath(); c.moveTo(-2 * s, -32 * s); c.lineTo(2 * s, -32 * s); c.lineTo(3 * s, -2 * s); c.lineTo(-1 * s, -2 * s); c.closePath(); c.fill(); c.restore();
  c.fillStyle = '#0a0b0e'; c.beginPath(); c.moveTo(1 * s, -2 * s); c.lineTo(7 * s, 0); c.lineTo(1 * s, 1 * s); c.closePath(); c.fill();
  const dg = c.createLinearGradient(-12 * s, 0, 12 * s, 0); dg.addColorStop(0, '#070809'); dg.addColorStop(0.46, '#23272f'); dg.addColorStop(0.6, '#14171c'); dg.addColorStop(1, '#070809');
  c.fillStyle = dg; c.shadowColor = 'rgba(0,0,0,0.6)'; c.shadowBlur = 12 * s;
  c.beginPath(); c.moveTo(-7 * s, -58 * s); c.quadraticCurveTo(-12 * s, -40 * s, -6 * s, -34 * s); c.quadraticCurveTo(-16 * s, -18 * s, -12 * s + walk * 2 * s, 0); c.quadraticCurveTo(0, 6 * s, 12 * s + walk * 2 * s, 0); c.quadraticCurveTo(16 * s, -18 * s, 6 * s, -34 * s); c.quadraticCurveTo(12 * s, -40 * s, 7 * s, -58 * s); c.closePath(); c.fill();
  c.beginPath(); c.moveTo(-7 * s, -58 * s); c.quadraticCurveTo(-8 * s, -70 * s, -5 * s, -76 * s); c.lineTo(5 * s, -76 * s); c.quadraticCurveTo(8 * s, -70 * s, 7 * s, -58 * s); c.closePath(); c.fill();
  c.shadowBlur = 0;
  c.fillStyle = PALETTE.bone;
  c.beginPath(); c.moveTo(-5 * s, -76 * s); c.quadraticCurveTo(0, -71 * s, 5 * s, -76 * s); c.lineTo(4 * s, -72 * s); c.quadraticCurveTo(0, -68 * s, -4 * s, -72 * s); c.closePath(); c.fill();
  c.beginPath(); c.moveTo(-5 * s, -76 * s); c.quadraticCurveTo(-12 * s, -72 * s, -10 * s, -50 * s); c.lineTo(-7 * s, -50 * s); c.quadraticCurveTo(-8 * s, -70 * s, -3 * s, -74 * s); c.closePath(); c.fill();
  c.beginPath(); c.arc(0, -84 * s, 6.5 * s, 0, TWO_PI); c.fill();
  c.fillStyle = '#050506'; c.beginPath(); c.arc(0, -86 * s, 8 * s, Math.PI * 0.85, Math.PI * 2.25); c.fill();
  c.beginPath(); c.moveTo(6.5 * s, -88 * s); c.quadraticCurveTo(13 * s, -76 * s, 8 * s, -62 * s); c.quadraticCurveTo(4 * s, -72 * s, 4 * s, -84 * s); c.closePath(); c.fill();
  c.beginPath(); c.moveTo(-6.5 * s, -88 * s); c.quadraticCurveTo(-12 * s, -78 * s, -8 * s, -66 * s); c.quadraticCurveTo(-4 * s, -74 * s, -4 * s, -84 * s); c.closePath(); c.fill();
  c.fillStyle = PALETTE.redHot; c.beginPath(); c.ellipse(0, -81 * s, 2.2 * s, 1.1 * s, 0, 0, TWO_PI); c.fill();
  c.restore();
});
