...
defineProp('coffee', function (e) {                           // a mug of coffee, steam rising (the cage)
  const c = e.ctx, s = e.scaleOf(this), X = e.X(this), y = (this.y != null ? this.y * e.H : e.gy) + (this.dy || 0) * e.unit, t = e.t;
  c.save(); c.globalCompositeOperation = 'lighter';
  for (let i = 0; i < 4; i++) { const life = (t * 0.5 + i * 0.25) % 1, yy = y - 13 * s - life * 24 * s, a = 0.10 * (1 - life); c.fillStyle = `rgba(220,222,228,${a})`; c.beginPath(); c.arc(X + Math.sin(t * 1.3 + i) * 3 * s, yy, (1.4 + life * 3) * s, 0, TWO_PI); c.fill(); }
  c.restore();
  c.fillStyle = '#16181d'; c.beginPath(); c.moveTo(X - 7 * s, y - 12 * s); c.lineTo(X + 7 * s, y - 12 * s); c.lineTo(X + 6 * s, y); c.lineTo(X - 6 * s, y); c.closePath(); c.fill();
  c.strokeStyle = '#2a2e36'; c.lineWidth = 1.4 * s; c.beginPath(); c.arc(X + 8 * s, y - 7 * s, 4 * s, -1.2, 1.2); c.stroke();
  c.fillStyle = '#0a0806'; c.beginPath(); c.ellipse(X, y - 12 * s, 6.5 * s, 1.8 * s, 0, 0, TWO_PI); c.fill();
  c.fillStyle = 'rgba(120,90,60,0.25)'; c.beginPath(); c.ellipse(X - 1.5 * s, y - 12.4 * s, 2.6 * s, 0.8 * s, 0, 0, TWO_PI); c.fill();
});
