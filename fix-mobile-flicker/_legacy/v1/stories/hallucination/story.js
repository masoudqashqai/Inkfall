/* =====================================================================================
   ...
   `export default` a story object; the manifest lazy-loads it on selection.

   Same grammar as every tale (see stories/SCHEMA.md): a story is { title, subtitle,
   blurb, audio?, scenes: [...] }. This is the original wordless-noir mood piece — a woman
   in red, a lit cigarette, two shots in an alley, and a rooftop where the rain lies about
   washing it clean.
   ===================================================================================== */
export default {
  title: 'INKFALL',
  ...
  // every sound this story uses — the engine plays nothing that isn't declared here
  audio: {
  ...
  scenes: [
    ...
    { // ---------------------------------------------------------------- 1 · THE STREET
      ...
      cast: [
        ...
        { actor: 'trafficLight', x: 0.90, scale: 0.9, par: 0.6, greenAt: 2 },
        { actor: 'redCar', x: 0.84, scale: 0.7, dy: -6, par: 0.5, walk: [0.84, 0.84, 1.45, 1.7], walkDur: 3.2 },
        { actor: 'womanInRed', x: 0.7, scale: 0.8, par: 0.4, walk: [0.72, 0.52, 0.30, -0.3], walkDur: 11, passX: 0.34 },
        { actor: 'trenchMan', x: 0.34, scale: 1, dy: 4, par: 0.5, raiseAt: 1, lightAt: 3 }
      ],
      script: [
    ...
    { // ---------------------------------------------------------------- 2 · THE ALLEY
      ...
      cast: [
        { actor: 'dumpster', x: 0.11, scale: 1, par: 0.4 },
        { actor: 'manhole', x: 0.6, scale: 1.1, par: 0.2 },
        { actor: 'steam', x: 0.6, seed: 0.9 },
        { actor: 'newspaper', x: 0, restX: 0.2, seed: 2.1 },
        { actor: 'gunman', x: 0.30, scale: 1, par: 0.5, raiseAt: 1 },
        { actor: 'bodyOnGround', x: 0.5, scale: 1, par: 0.5, onFlag: 'blood' },
        { actor: 'bloodSplat', x: 0.5, onFlag: 'blood', seed: 999 },
        { actor: 'bloodDrain', x: 0.5, drainX: 0.6, par: 0.2, onFlag: 'blood', drainAt: 3 }
      ],
      script: [
    ...
    { // ---------------------------------------------------------------- 3 · THE ROOFTOP
      ...
      lights: [
        { type: 'glow', x: 0.5, y: 0.7, r: 220, color: 'rgba(190,200,220,ALPHA)', intensity: 0.7 }
      ],
      cast: [
        { actor: 'searchlight', x: 0.5 },
        { actor: 'waterTower', x: 0.18, scale: 1, par: 0.4 },
        { actor: 'crow', x: 0.18, y: 0.43, scale: 1.5, par: 0.4, flyAt: 3, delay: 0.0 },
        { actor: 'crow', x: 0.36, y: 0.25, scale: 1.1, flyAt: 3, delay: 0.14 },
        { actor: 'crow', x: 0.67, y: 0.22, scale: 1.0, flyAt: 3, delay: 0.26 },
        { actor: 'steam', x: 0.74, seed: 0.5 },
